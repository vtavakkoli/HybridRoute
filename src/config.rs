...
impl AppConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        anyhow::ensure!(
            self.retrieval.candidate_limit > 0,
            "candidate_limit must be positive"
        );
        anyhow::ensure!(self.retrieval.ann_tables > 0, "ann_tables must be positive");
        anyhow::ensure!(
            self.retrieval.ann_bits_per_table > 0,
            "ann_bits_per_table must be positive"
        );
        ...
        anyhow::ensure!(
            self.adaptation.learning_rate >= 0.0 && self.adaptation.learning_rate <= 1.0,
            "learning_rate must be in [0,1]"
        );
        anyhow::ensure!(
            self.adaptation.max_step > 0.0 && self.adaptation.max_step <= 0.25,
            "max_step must be in (0,0.25]"
        );
        ...
        for route in &self.routes {
            ...
            anyhow::ensure!(
                (0.0..=1.0).contains(&route.quality),
                "route {} quality must be in [0,1]",
                route.id
            );
            if route.fallback {
                fallback_count += 1;
            ...
            if route.high_impact {
                anyhow::ensure!(
                    !route.safe_for_exploration,
                    "high-impact route {} cannot enable exploration",
                    route.id
                );
                anyhow::ensure!(
                    !route.allow_adaptation,
                    "high-impact route {} cannot enable adaptation",
                    route.id
                );
            }
            if route.fallback {
                anyhow::ensure!(
                    !route.allow_adaptation,
                    "fallback route {} cannot enable adaptation",
                    route.id
                );
            }
        }
...
impl Default for ServerConfig {
    fn default() -> Self {
        Self {
            ...
            json_logs: false,
            config_reload_debounce_ms: default_reload_ms(),
        }
    }
}
...
impl Default for RetrievalConfig {
    fn default() -> Self {
        Self {
            ...
            ann_tables: default_ann_tables(),
            ann_bits_per_table: default_ann_bits(),
            ann_probe_radius: default_ann_probe_radius(),
        }
    }
}
...
impl Default for ScoringConfig {
    fn default() -> Self {
        Self {
            ...
            schema_weight: default_schema_weight(),
...
impl Default for HealthConfig {
    fn default() -> Self {
        Self {
            ...
            circuit_open_ms: default_open_ms(),
...
impl Default for AdaptationConfig {
    fn default() -> Self {
        Self {
            ...
            feedback_token_env: default_feedback_token_env(),
        }
    }
}
...
fn default_ann_tables() -> usize {
    4
}
fn default_ann_bits() -> usize {
    12
}
fn default_ann_probe_radius() -> u8 {
    1
}
...
fn default_max_quality() -> f32 {
    0.98
}
fn default_min_feedback_samples() -> u64 {
    3
}
fn default_feedback_token_env() -> String {
    "HYBRIDROUTE_FEEDBACK_TOKEN".into()
}
