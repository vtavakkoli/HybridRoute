...
impl RetrievalIndex {
    pub fn build(config: RetrievalConfig, documents: Vec<RetrievalDocument>) -> Self {
        ...
        for document in &documents {
            ...
            for token in unique { *document_frequency.entry(token).or_default() += 1; }
            if let Some(vector) = &document.vector {
                for (table, buckets) in lsh.iter_mut().enumerate() {
                    ...
                    buckets.entry(signature).or_default().push(document.route_index);
                }
            }
        }
        let average_length = if documents.is_empty() { 1.0 } else { total_length as f32 / documents.len() as f32 };
        Self { config, documents, document_frequency, average_length: average_length.max(1.0), lsh }
    }
    ...
    pub fn candidates(&self, query_tokens: &[String], query_vector: Option<&[f32]>) -> Vec<usize> {
        ...
        let mut selected = scores.into_iter().take(self.config.candidate_limit).map(|(index, _)| index).collect::<HashSet<_>>();
    ...
    pub fn bm25_score(&self, route_index: usize, query_tokens: &[String]) -> f32 {
        let Some(document) = self.documents.iter().find(|doc| doc.route_index == route_index) else { return 0.0; };
        self.score_document(document, query_tokens)
    }

    fn bm25_scores(&self, query_tokens: &[String]) -> Vec<(usize, f32)> {
        self.documents.iter().map(|doc| (doc.route_index, self.score_document(doc, query_tokens))).collect()
    }

    fn score_document(&self, document: &RetrievalDocument, query_tokens: &[String]) -> f32 {
        if query_tokens.is_empty() || document.tokens.is_empty() { return 0.0; }
        let mut frequencies = HashMap::<&str, usize>::new();
        for token in &document.tokens { *frequencies.entry(token.as_str()).or_default() += 1; }
        ...
        for token in query_tokens.iter().collect::<HashSet<_>>() {
            ...
            let denominator = tf + self.config.bm25_k1 * (1.0 - self.config.bm25_b + self.config.bm25_b * dl / self.average_length);
...
fn add_bucket(buckets: &HashMap<u64, Vec<usize>>, key: u64, selected: &mut HashSet<usize>) {
    if let Some(indices) = buckets.get(&key) { selected.extend(indices.iter().copied()); }
}
...
fn signature(vector: &[f32], table: usize, bits: usize) -> u64 {
    ...
    for bit in 0..bits.min(63) {
        ...
        for (dimension, value) in vector.iter().enumerate() {
            ...
            let sign = if hash.as_bytes()[0] & 1 == 0 { 1.0 } else { -1.0 };
            projection += value * sign;
        }
        if projection >= 0.0 { output |= 1u64 << bit; }
    }
    output
}
...
mod tests {
    ...
    fn bm25_prefers_matching_document() {
        let index = RetrievalIndex::build(RetrievalConfig::default(), vec![
            RetrievalDocument { route_index: 0, tokens: vec!["streetlight".into(), "broken".into()], vector: None },
            RetrievalDocument { route_index: 1, tokens: vec!["parking".into(), "permit".into()], vector: None },
        ]);
        assert!(index.bm25_score(0, &["streetlight".into()]) > index.bm25_score(1, &["streetlight".into()]));
    }
}
