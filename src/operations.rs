...
impl OperationalState {
    ...
    pub fn register_routes(&self, routes: &[RouteConfig]) {
        for route in routes {
            self.routes.entry(route.id.clone()).or_insert_with(|| {
                Arc::new(Mutex::new(RouteOperationalState {
                    healthy: true,
    ...
    pub async fn eligible(&self, route_id: &str) -> bool {
        let Some(entry) = self.routes.get(route_id).map(|v| Arc::clone(v.value())) else {
            return true;
        };
        let mut state = entry.lock().await;
        if let Some(opened) = state.circuit_opened_at {
            if opened.elapsed() < Duration::from_millis(self.health.circuit_open_ms) {
                return false;
            }
            if state.half_open_in_flight >= self.health.half_open_max_requests {
                return false;
            }
    ...
    pub async fn quality(&self, route_id: &str, configured: f32) -> f32 {
        ...
        let quality = entry.lock().await.quality;
        quality
    }

    pub async fn state_view(&self, route_id: &str) -> (bool, CircuitStateView) {
        let Some(entry) = self.routes.get(route_id).map(|v| Arc::clone(v.value())) else {
            return (true, CircuitStateView::Closed);
        };
    ...
    pub async fn adapt(&self, route: &RouteConfig, reward: f32) -> anyhow::Result<(f32, u64)> {
        ...
        anyhow::ensure!(
            ...
            "adaptation is disabled for this route"
        );
        ...
        anyhow::ensure!(
            reward.is_finite() && (-1.0..=1.0).contains(&reward),
        ...
        if state.feedback_samples >= self.adaptation.min_feedback_samples {
            let target = ((reward + 1.0) * 0.5)
                .clamp(self.adaptation.min_quality, self.adaptation.max_quality);
            ...
            state.quality = (state.quality + step)
                .clamp(self.adaptation.min_quality, self.adaptation.max_quality);
        }
    ...
    pub async fn snapshots(&self) -> Vec<RouteOperationalSnapshot> {
        ...
        for (route_id, entry) in entries {
            ...
            output.push(RouteOperationalSnapshot {
                route_id,
