...
impl AppState {
    pub fn new(runtime: RuntimeManager) -> Result<Self> {
        ...
        let client = Client::builder()
            .connect_timeout(Duration::from_millis(table.config.proxy.connect_timeout_ms))
            .timeout(Duration::from_millis(
                table.config.proxy.upstream_timeout_ms,
            ))
            .build()?;
        Ok(Self {
            engine: RouterEngine::new(runtime.clone()),
            runtime,
            client,
        })
    }
...
pub async fn decision_api(
    ...
    let context = RoutingContext {
        ...
        top_k: request.top_k.unwrap_or(table.config.decision.top_k),
    };
...
pub async fn proxy_request(
    ...
    let body = axum::body::to_bytes(body, table.config.server.max_body_bytes)
        .await
    ...
    let content_type = parts
        ...
        .map(str::to_string);
    let (text, json_body) = extract_routing_input(
        &table.config,
        &parts.headers,
    ...
    if text.trim().is_empty() {
        return Err(ApiError::unprocessable("no routing text found"));
    }
    let headers = parts
        .headers
        .iter()
    ...
    let sticky_key = string_header(&parts.headers, &table.config.extraction.sticky_header)
        .unwrap_or_else(|| uuid::Uuid::new_v4().to_string());
    let context = RoutingContext {
        text,
        ...
        body: json_body,
        sticky_key,
        top_k: table.config.decision.top_k,
    };
    let decision = state.engine.decide(&context).await?;
    if let Some(clarification) = decision.clarification {
        return Ok((StatusCode::CONFLICT, Json(clarification)).into_response());
    }
    let selected = decision
        .selected
        .ok_or_else(|| ApiError::unprocessable("no eligible route"))?;
    let target = build_target_url(
        &selected.target,
        selected.rewrite_path.as_deref(),
        parts.uri.path(),
        if table.config.proxy.preserve_query {
            parts.uri.query()
        } else {
            None
        },
    ...
    if table.config.proxy.add_decision_headers {
        upstream = upstream
            ...
            .header("x-hybridroute-generation", decision.generation.to_string());
    }
    let upstream = match upstream.body(body).send().await {
        Ok(response) => response,
        Err(error) => {
            state.runtime.metrics.upstream_failures.inc();
            state
                .runtime
                .operations
                .record_failure(&selected.route_id)
                .await;
    ...
    if status.is_server_error() {
        ...
        state
            .runtime
            .operations
            .record_success(&selected.route_id)
            .await;
    }
    ...
    for (name, value) in &upstream_headers {
        if !is_hop_by_hop(name) && !is_internal(name) && name != header::CONTENT_LENGTH {
    ...
    response
        ...
        .map_err(|e| ApiError::internal(format!("failed to construct response: {e}")))
}

fn extract_routing_input(
    ...
    if content_type.is_some_and(|v| v.to_lowercase().starts_with("text/")) {
        let text = std::str::from_utf8(body)
            .map_err(|_| ApiError::bad_request("text body is not UTF-8"))?;
        return Ok((
            text.chars()
                .take(config.extraction.max_semantic_chars)
                .collect(),
...
fn build_target_url(
    ...
    let mut url =
        Url::parse(base).map_err(|e| ApiError::internal(format!("invalid target URL: {e}")))?;
    url.set_path(rewrite.unwrap_or(original));
    url.set_query(query);
    Ok(url)
}
...
impl From<anyhow::Error> for ApiError {
    fn from(error: anyhow::Error) -> Self {
        tracing::error!(%error, "internal routing error");
